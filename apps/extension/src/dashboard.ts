...
async function leaveSelected(): Promise<void> { const selected = Array.from(document.querySelectorAll<HTMLInputElement>('[data-guild]:checked')).map((input) => guilds.find((guild) => guild.id === input.dataset.guild)).filter((guild): guild is Guild => Boolean(guild && !guild.owner)); if (!selected.length) { notice = 'Select at least one leaveable server.'; render(); return; } if (!await dashboardDialog({ title: 'Leave selected servers?', message: 'You are about to leave ' + selected.length + ' server(s). This cannot be undone.', confirmLabel: 'Leave servers', requiredText: 'LEAVE', danger: true })) return; for (const guild of selected) { try { await leaveOneGuild(guild); } catch (error) { notice = 'Error leaving ' + guild.name + ': ' + errorText(error); } } await refresh(); render(); }
